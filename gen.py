...
for path in pathlist:
    ...
    file_name_we = "temp_files/" + \
        str(ntpath.basename(path_in_str).replace(".", "-")) + ".html"
    ...
    f.write((highlight(code, lexer, HtmlFormatter(
        full=True, highlight=sys.argv[3], linenos=True))))
    f.close()
    # gen png
    file_name_we_for_img = str(
        sys.argv[2]) + "/" + str(ntpath.basename(path_in_str).replace(".", "-")) + ".png"
    imgkit.from_file(file_name_we, file_name_we_for_img)
for i in Path("temp_files").glob("./*.*"):
    os.remove(i)
os.removedirs("temp_files")